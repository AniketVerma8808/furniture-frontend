...
export default function YourOrder() {
    const { getPendingOrders, markOrderAsDelivered, orders } = useContext(ProductContext);
                                        ...
                                        <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                                            <div className="flex items-center justify-between md:order-3 md:justify-end">
                                                <div className="text-end md:order-4 md:w-32">
                                                    <p className="text-base font-bold text-gray-900 dummy:text-white">Price: ₹{item.totalPrice}</p>
                                                    <p className="text-sm font-medium text-gray-600 dummy:text-gray-400">Payment: {item.paymentMethod}</p>
                                                </div>
                                            </div>
                                            <div>
                                                <img src={item.cart[0].images[0]} alt={item.cart[0].title} className="w-32" />
                                            </div>
                                            <div className="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
                                                <h1 className="text-base font-medium text-gray-900 dummy:text-white">{item.cart[0].title}</h1>
                                            </div>
                                        </div>
                                    </div>
                                ))}
